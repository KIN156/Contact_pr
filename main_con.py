...
def con_main():
	...
	while ch != 0:
		...
		if ch == 1:
			...
			contact_methods.add_data(db,name,phone_no)
		elif ch == 2:
			s = input('Write number or name to delete contact : ')
			contact_methods.delete_data(db,s)
		elif ch == 3:
			contact_methods.view_data(db)
		elif ch == 4:
			s = input('Write phone number or name to edit contact : ')
			contact_methods.edit_data(db,s)
		elif ch == 5:
			s = input('Write number or name to edit contact : ')
			contact_methods.search(db,s)
		elif ch == 0:
